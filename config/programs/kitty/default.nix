...
ctx.pkgs.kitty.override {
  python3Packages = ctx.pkgs.python3Packages // {
    buildPythonApplication = cfg:
      ctx.pkgs.python3Packages.buildPythonApplication (cfg // {
        configurePhase = ''
          ${cfg.configurePhase}
        ...
        installPhase = let
          iosevka =
            "${ctx.iosevka}/share/fonts/truetype/Iosevkacustom-${ctx.terminal-settings.weight}.ttf";
        in ''
          ${cfg.installPhase}
        ...
        nativeBuildInputs = cfg.nativeBuildInputs
          ++ (with ctx.pkgs.python3Packages; [ matplotlib ]);
      });
  };
}
